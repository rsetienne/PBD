  ...
  verbose = TRUE)
{
  ...
  set.seed(seed)
  if (verbose) cat('Finding the maximum likelihood estimates ...\n\n')
  if(plotltt)
  {
    plot(c(-brts,0),c(soc:(length(brts)+soc-1),length(brts)+soc-1),log = 'y',type = 's',xlab = 'Time',ylab = 'Number of lineages')
  }
  empout = pbd_ML(brts,initparsopt,idparsopt,idparsfix,parsfix,exteq,parsfunc,missnumspec,cond,btorph,soc,methode,n_low,n_up,tol,maxiter,verbose = verbose)
  ...
  for(i in 1:endmc)
  {
    if (verbose) cat('Simulated data set',i,'out of',endmc,'\n')
    flush.console()
    simbrts = pbd_sim_cpp(pars = MLpars, age = max(abs(brts)), soc = soc, plotltt = plotltt, methode = methode)
    trees[[i]] = brts2phylo(simbrts)
    simML = pbd_ML(simbrts,initparsopt = MLpars[1:(4-exteq)],idparsopt,idparsfix,parsfix,exteq,parsfunc,missnumspec = 0,cond,btorph,soc,methode,n_low,n_up,tol,maxiter,verbose = verbose)
